...
async function IndexJS(req, res) {
    ...
    let config = {
        ...
        , auth: `${getProtocol(req)}://${req.get('host')}${req.baseUrl}${res.locals.nav.url}`
        , angular_client: res.locals.nav
    }
    return `__CONFIG__ = Object.assign(__CONFIG__, ${JSON.stringify(config)})`
}
