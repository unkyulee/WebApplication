...
export class NavService {
    ...
    find(url: string) {
        ...
        for (const navItem of this.navigation) {
            ...
            if (firstNavItem == null && navItem.type == 'item') firstNavItem = navItem

            // if has children then loop through children
            if (navItem.children) {
                for (const child of navItem.children) {
                    // save first navigation item
                    if (firstNavItem == null && child.type == 'item') firstNavItem = child

                    if (child.url && url.startsWith(child.url))
                        return child
                }
            }

            // return found item
            if (url.startsWith(navItem.url))
                return navItem
        }
    ...
    back() {
        ...
        if (this.navgiationStack.length > 0)
            this.router.navigateByUrl(this.navgiationStack[this.navgiationStack.length - 1])
        else
            this.router.navigateByUrl(this.currNav.url)
    }
